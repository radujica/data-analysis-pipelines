...
parser.add_argument('-p', '--partitions', required=True, help='Number of partitions', type=int)
...
df1 = dd.from_pandas(to_dataframe(PATH1).reset_index(), npartitions=1)
df2 = dd.from_pandas(to_dataframe(PATH2).reset_index(), npartitions=1)

print_event('done_read')


def save_csv(dataframe, name, index=True):
    dataframe.to_csv(args.output + name + '.csv', sep=',', header=True, index=index)


"PIPELINE"
# 1. join the 2 dataframes
df = df1.merge(df2, on=['longitude', 'latitude', 'time'], how='inner')
...
save_csv(df_head, 'head', index=False)

# 3. want a subset of the data, approx. 33%
slice_ = [int(x) for x in args.slice.split(':')]
# slice doesn't seem to work on multiple partitions, which kinda makes sense but a warning would be nice
df = df[slice_[0]:slice_[1]]
df = df.repartition(npartitions=args.partitions)
...
df = df.drop(['pp_stderr', 'rr_stderr'], axis=1)
...
df_agg = df.drop(['longitude', 'latitude', 'time'], axis=1).describe().reset_index()
df_agg = df_agg[df_agg['index'].isin(aggs)].rename(columns={'index': 'agg'}).set_index('agg').compute()

print_event('done_agg')

save_csv(df_agg, 'agg', index=True)
