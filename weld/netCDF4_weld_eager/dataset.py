...
class Dataset(LazyFile):
    ...
    def _create_variables(self):
        ...
        for kv in self.ds.variables.items():
            ...
            weld_obj, weld_input_name = LazyResult.generate_placeholder_weld_object(data_id,
                                                                                    Variable.encoder,
                                                                                    Variable.decoder)

            variable = Variable(self.file_id, kv[0], kv[1].dimensions, kv[1].shape,
                                kv[1].__dict__, weld_obj, kv[1].dtype)
            LazyResult.register_lazy_data(weld_input_name, variable)

            # force read it eagerly
            LazyResult.input_mapping[str(weld_input_name)] = variable.eager_read()
